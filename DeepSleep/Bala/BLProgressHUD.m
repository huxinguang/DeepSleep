...
#define BLMainThreadAssert() NSAssert([NSThread isMainThread], @"BLProgressHUD needs to be accessed on the main thread.");

CGFloat const BLProgressMaxOffset = 1000000.f;

static const CGFloat BLDefaultPadding = 4.f;
static const CGFloat BLDefaultLabelFontSize = 16.f;
static const CGFloat BLDefaultDetailsLabelFontSize = 12.f;


@interface BLProgressHUD () {
    // Deprecated
...
@interface BLProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    BLProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    BLProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (BLProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (BLProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = BLProgressHUDAnimationFade;
    _mode = BLProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(BLProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == BLProgressHUDAnimationZoom) {
        type = animatingIn ? BLProgressHUDAnimationZoomIn : BLProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == BLProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == BLProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == BLProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == BLProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    BLProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<BLProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    BLBackgroundView *backgroundView = [[BLBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = BLProgressHUDBackgroundStyleSolidColor;
    ...
    BLBackgroundView *bezelView = [BLBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:BLDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:BLDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [BLProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:BLDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[BLRoundProgressView class]];

    BLProgressHUDMode mode = self.mode;
    if (mode == BLProgressHUDModeIndeterminate) {
    ...
    else if (mode == BLProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[BLBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == BLProgressHUDModeDeterminate || mode == BLProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[BLRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == BLProgressHUDModeAnnularDeterminate) {
            [(BLRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == BLProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == BLProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[BLProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[BLProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[BLRoundProgressView class]]) {
        BLRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [BLRoundProgressView appearanceWhenContainedIn:[BLProgressHUD class], nil];
#else
        appearance = [BLRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[BLProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((BLRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((BLRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[BLBarProgressView class]]) {
        BLBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [BLBarProgressView appearanceWhenContainedIn:[BLProgressHUD class], nil];
#else
        appearance = [BLBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[BLProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((BLBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((BLBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    BLBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? BLDefaultPadding : 0.f;
...
- (void)setMode:(BLProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == BLProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation BLRoundProgressView
...
@implementation BLBarProgressView
...
@implementation BLBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = BLProgressHUDBackgroundStyleBlur;
            ...
            _style = BLProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(BLProgressHUDBackgroundStyle)style {
    if (style == BLProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = BLProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    BLProgressHUDBackgroundStyle style = self.style;
    if (style == BLProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == BLProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation BLProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [BLProgressHUD allHUDsForView:view];
    for (BLProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable BLProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    BLBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == BLProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    BLMainThreadAssert();
    self.backgroundView.style = BLProgressHUDBackgroundStyleSolidColor;
...
@implementation BLProgressHUDRoundedButton
